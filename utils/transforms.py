...
def transform_train(resize_size=256, crop_size=224):
    ...
    return transforms.Compose([
        ...
        transforms.RandomResizedCrop(crop_size, scale=(0.08, 1.0), interpolation=transforms.InterpolationMode.BICUBIC),
...
def transform_test(resize_size=256, crop_size=224):
    ...
    data_transforms = {'test0': transforms.Compose([
        ...
        transforms.ToTensor(),
        normalize
    ]), 'test6': transforms.Compose([
        ResizeImage(resize_size),
        PlaceCrop(crop_size, start_last, start_last),
        transforms.ToTensor(),
        normalize
    ]), 'test7': transforms.Compose([
        ResizeImage(resize_size),
        PlaceCrop(crop_size, start_last, start_first),
        transforms.ToTensor(),
        normalize
    ]), 'test8': transforms.Compose([
        ResizeImage(resize_size),
        PlaceCrop(crop_size, start_first, start_last),
        transforms.ToTensor(),
        normalize
    ]), 'test9': transforms.Compose([
        ResizeImage(resize_size),
        PlaceCrop(crop_size, start_center, start_center),
        transforms.ToTensor(),
        normalize
